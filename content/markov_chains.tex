...
As the purpose of \projectname{} is to be able to predict the movement of bikes at a given time (see \cref{problem_statement}), it must be able to answer questions such as the following:
\begin{itemize}
...
\begin{itemize}
\item How much time will it take before a bike will be \textit{in a given hotspot}?
\item What is the probability of finding a bike \textit{in a given hotspot} from 16.25 to 16.35?
\stefan{can we answer this?}
\end{itemize}

To answer these questions we need to model the behavior of an \emph{average} bike.
