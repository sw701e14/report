...
The clustering algorithm chosen, DBSCAN explained in \Cref{clustering:DBSCAN}, expects two paramters, $eps$ and $MinPts$.
...
Setting $eps \leftarrow 15$ makes sense because a GPS data point then can be up to 15 metres away from its real location.
...
One should keep in mind that the DBSCAN paramters, which worked with the constructed data set, may need to be calibrated when used on real data.
