...
We have chosen the solution with the same bikes as the current system, with the addition of a GPS receiver\footnote{A GPS receiver is a unit which uses the GPS to calculate its location. The GPS receiver is presumed to send an accuracy in metres like when using this service, \cite{followme}.} on each individual bike, and completely removing the stations and bike locks.
...
\subparagraph{\ref{pr_stations}: Bikes left outside stations} is handled by the GPS receivers, as bikes and their location now can be found, no matter if it is at a hotspot or not.
