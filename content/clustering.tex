...
\mikkel{What is an acceptable clustering?}
...
\mikkel{Clusterings has been mentioned before this definition of the word.}
...
\mikkel{Enig. Vi burde have en præcis definition og så anvende den efterfølgende.}
...
\mikkel{Skal vi ikke bare skrive det om til at passe med det vi faktisk gør?}
...
\mikkel{Som jeg læser det her lægger det op til complete clustering. Er det ikke problematisk da vi vil forvente at have støj i vort data.}
...
\mikkel{Jeg synes godt vi kan benytte minimum nearest neighbours som et argument for DBSCAN.}
...
\mikkel{Jeg synes beskrivelsen af core point er lidt rodet/uklar}
...
\end{algorithm}
\mikkel{Skal vi gøre noget ved algoritmen ift notationen så vi undgår den uklarhed der er på wiki siden?}
...
\subsection{Summary}\bruno{Hvordan argumentere vi for at bruge DBSCAN fremfor en algoritme der møder alle vores krav?} Of the three presented algorithms, only the DBSCAN algorithm fits our need for an exclusive, partial and density-based clustering algorithm.
We therefore use this algorithm to detect clusters in our data, to be used as the hotspots in our model.
\mikkel{Jeg ville gerne have noget af det her nævnt inden vi går i detaljer med DBSCAN.
Jeg synes godt vi kan afslutte den her section med en subsection of DBSCAN, efter at have sagt at det er den vi vil bruge.}
...
\mikkel{Kan vi ikke bare sige hvad vi har gjort fremfor hvad vi gerne ville have gjort?}
