...
\node[above=of updater.north east,yshift=-1cm,xshift=0.7cm](persistency){Database};

\draw ($ (persistency.north west) + (-0.3,0.3) $) rectangle ($ (data.east)+ (database.south) - (data.south) +(0.3,-.9) $);

% Data import
\node[draw,minimum width=4cm,right=of model,xshift=0.5cm](datacollector){Data Collector};
