...
\While{$d \leq l$}
	...
	\ForEach{$b \in B$}
		{
		$g \leftarrow G(b, d)$\\
\HiLi		$n \leftarrow \textbf{NearestState}(H, g)$\\
\HiLi		\If{$n \in D \wedge L(b) \in D$}
			{
\HiLi		$n \leftarrow L(b)$
			}
\HiLi		\ElseIf{$n \in D \wedge L(b) \in H$}
			{
\HiLi			$n \leftarrow L(b).departure$
			}
		}
